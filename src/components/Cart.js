...
function Cart() {
    ...
    const getProducts = async () => {
        ...
        setEmail(email)
        const token = localStorage.getItem('token')
        let result = await fetch('http://13.49.225.235/api/v1/products/getProductsInCart', {
            method: "POST",
            body: JSON.stringify({
                email,
            }),
    ...
    const removeFromCart = async (id) => {
        ...
        const email = localStorage.getItem(`userEmail`)
        const productId = id
        const response = await fetch("http://13.49.225.235/api/v1/products/removeFromCart", {
            method: "POST",
            body: JSON.stringify({
                productId,
                email,
    ...
    const paymentHandler = async (e) => {
        setShowPayment(false)
        const response = await fetch("http://13.49.225.235/api/v1/payment/order", {
            method: "POST",
            body: JSON.stringify({
                email,
                amount,
        ...
        var options = {
            ...
            handler: async function (response) {
                ...
                const validateRes = await fetch(
                    "http://13.49.225.235/api/v1/payment/order/validate",
                    {
                        method: "POST",
    ...
    const confirmOrder = async () => {
        ...
        const email = localStorage.getItem("userEmail")
        const token = localStorage.getItem("token")
        const response = await fetch("http://13.49.225.235/api/v1/payment/confirm", {
            method: "POST",
            body: JSON.stringify({
                email,
                products,
