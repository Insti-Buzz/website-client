...
function AllOrders() {
    ...
    const getProducts = async () => {
        ...
        const token = localStorage.getItem("token")
        console.log(email)
        let result = await fetch('http://3.110.81.106/api/v1/products/allOrders', {
            method: 'POST',
    ...
    function e(item, index) {
        ...
        const deliveryDone = async () => {
            const token = localStorage.getItem("token")
            let result = await fetch('http://3.110.81.106/api/v1/products/delivered', {
                method: 'POST',
